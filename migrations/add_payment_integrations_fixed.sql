...
SET @sql = 'CREATE INDEX idx_saved_payment_provider ON saved_payment_methods(user_id, provider)';
SET @sql_check = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = 'blindscommerce' AND TABLE_NAME = 'saved_payment_methods' AND INDEX_NAME = 'idx_saved_payment_provider');
SET @sql_final = IF(@sql_check = 0, @sql, 'SELECT ''Index idx_saved_payment_provider already exists'' as message');
PREPARE stmt FROM @sql_final; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @sql = 'CREATE INDEX idx_saved_payment_external ON saved_payment_methods(provider, external_id)';
SET @sql_check = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = 'blindscommerce' AND TABLE_NAME = 'saved_payment_methods' AND INDEX_NAME = 'idx_saved_payment_external');
SET @sql_final = IF(@sql_check = 0, @sql, 'SELECT ''Index idx_saved_payment_external already exists'' as message');
PREPARE stmt FROM @sql_final; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- Update payments table to support new payment methods (payment_method is already updated)
...
SELECT 'Payment integrations created successfully!' as Status;