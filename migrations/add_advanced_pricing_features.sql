...
BEGIN
    IF NEW.price != OLD.price THEN
        INSERT INTO product_pricing_history (
            ...
            NEW.product_id, OLD.price, NEW.price, 'manual_update', 
            'system', NOW()
        );
    END IF;
END$$
...
SET @sql = (SELECT IF(
    ...
    'ALTER TABLE orders ADD COLUMN discount_amount DECIMAL(10,2) DEFAULT 0.00 COMMENT "Total discount applied"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
     ...
     AND TABLE_SCHEMA = 'blindscommerce' 
     AND TABLE_NAME = 'orders') > 0,
    'SELECT "Foreign key fk_orders_coupon already exists" as Info',
...
SELECT 'Advanced Pricing Features system created successfully!' as Status;