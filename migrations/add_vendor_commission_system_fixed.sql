...
-- Note: Commission calculation trigger disabled until vendor_id and sales_staff_id columns are added to orders table
-- This trigger will be created after the necessary columns are added

-- Note: Vendor discount trigger disabled until vendor_discount_id column is added to orders table
-- This trigger will be created after the necessary columns are added
...
-- Add vendor_id column to orders table if it doesn't exist
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'blindscommerce' 
     AND TABLE_NAME = 'orders' 
     AND COLUMN_NAME = 'vendor_id') > 0,
    'SELECT "Column vendor_id already exists" as Info',
    'ALTER TABLE orders ADD COLUMN vendor_id INT NULL COMMENT "Reference to vendor_info table"'
));
...
-- Add foreign key for vendor_id if it doesn't exist
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
     WHERE CONSTRAINT_NAME = 'fk_orders_vendor' 
     AND TABLE_SCHEMA = 'blindscommerce' 
     AND TABLE_NAME = 'orders') > 0,
    'SELECT "Foreign key fk_orders_vendor already exists" as Info',
