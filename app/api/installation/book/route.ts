...
export async function POST(req: NextRequest) {
  try {
    ...
    const [slotRows] = await pool.execute<TimeSlotRow[]>(
    ...
    const [serviceAreaRows] = await pool.execute<ServiceAreaRow[]>(
    ...
    if (assignedTechnicianId) {
      ...
      const [techAvail] = await pool.execute<RowDataPacket[]>(
      ...
      const [availableTechs] = await pool.execute<TechnicianRow[]>(
        `SELECT t.*, COUNT(a.appointment_id) as current_appointments,
                CASE 
    ...
    const [appointmentDetails] = await pool.execute<RowDataPacket[]>(
      `SELECT 
        a.*, 
